...
app.post('/iniciar-sesion', async (req, res) => {
    ...
    try {
        ...
        // 1. Primero verificar en usuarios ventas
        if (SPREADSHEET_SUPER_VENTAS) {
            try {
                const ventasResponse = await sheets.spreadsheets.values.get({
                    spreadsheetId: SPREADSHEET_SUPER_VENTAS,
                    range: 'Usuarios!A2:H'
                });

                const usuariosVentas = ventasResponse.data.values || [];
                const usuarioVentas = usuariosVentas.find(row =>
                    row && row.length >= 8 &&
                    row[4] === email &&        // EMAIL
                    row[5] === password &&     // CONTRASEÑA
                    row[3] && row[3].toLowerCase() === 'activo' // ESTADO
                );

                if (usuarioVentas) {
                    const token = jwt.sign(
                        {
                            email: usuarioVentas[4],
                            nombre: usuarioVentas[1],
                            spreadsheetId: usuarioVentas[7], // SPREADSHEET
                            tipo: 'ventas'
                        },
                        JWT_SECRET,
                        { expiresIn: '89280h' }
                ...
                console.error('Error verificando usuarios ventas:', ventasError);
            }
        }

        // 2. Si no es usuario ventas, verificar en empresas normales
        ...
        for (const row of empresas) {
            const spreadsheetIdEmpresa = row[6];
            ...
            try {
                ...
                const usuario = rows.find(row =>
                    ...
                    row[7] === email &&
                    row[8] === password &&
                    row[3] && row[3].toLowerCase() === 'activo'
                );
...
app.post('/subir-almacen-excel', requireAuth, upload.single('archivo'), async (req, res) => {
    ...
    try {
        ...
        if (data.length < 2) {
            return res.status(400).json({
                success: false,
                error: 'El archivo debe contener al menos una fila de datos (sin contar el encabezado)'
            });
        }
        ...
        const columnIndexes = {
            ...
            gramos: headers.findIndex(h => h === 'GR.'),
            stock: headers.findIndex(h => h === 'STOCK'),
            cantidadxgrupo: headers.findIndex(h => h === 'GRUP'),
            lista: headers.findIndex(h => h === 'LISTA'),
            codigo_barras: headers.findIndex(h => h === 'C. BARRAS'),
            precios: headers.findIndex(h => h === 'PRECIOS'),
            etiquetas: headers.findIndex(h => h === 'ETIQUETAS'),
            acopio_id: headers.findIndex(h => h === 'ACOPIO ID'),
            alm_acopio_producto: headers.findIndex(h => h === 'ALM-ACOPIO NOMBRE'),
            imagen: headers.findIndex(h => h === 'IMAGEN'),
            uSueltas: headers.findIndex(h => h === 'U SUELTAS'),
            promocion: headers.findIndex(h => h === 'PROMOCION'),
            precio_promocion: headers.findIndex(h => h === 'PRECIO PROMOCION')
        };

        // Verificar que las columnas requeridas estén presentes
        const columnasRequeridas = ['ID', 'PRODUCTO', 'GR.', 'STOCK'];
        const columnasFaltantes = columnasRequeridas.filter(col => 
            headers.findIndex(h => h === col) === -1
        );
        
        if (columnasFaltantes.length > 0) {
            return res.status(400).json({
                success: false,
                error: `El archivo debe contener las siguientes columnas: ${columnasFaltantes.join(', ')}`
            });
        }

        // Validar formato de IDs y verificar duplicados
        const idsEnArchivo = [];
        const idsDuplicados = [];
        const idsFormatoIncorrecto = [];
        
        for (let i = 1; i < data.length; i++) {
            const row = data[i];
            const id = row[columnIndexes.id];
        ...
        if (idsDuplicados.length > 0) {
            return res.status(400).json({
                success: false,
                error: `Los siguientes IDs están duplicados en el archivo: ${idsDuplicados.join(', ')}`
            });
        }

        // Obtener todos los productos existentes para buscar por ID
        const existingProductsResponse = await sheets.spreadsheets.values.get({
            spreadsheetId,
            range: 'Almacen general!A2:O'
        });

        const existingRows = existingProductsResponse.data.values || [];
        const existingProductsMap = new Map();
        
        existingRows.forEach((row, index) => {
            if (row[0]) { // Si tiene ID
                existingProductsMap.set(row[0], index + 2); // +2 porque empezamos desde A2
            }
        });
        ...
        // Procesar cada fila de datos (excluyendo encabezados)
        for (let i = 1; i < data.length; i++) {
            const row = data[i];
            const id = row[columnIndexes.id];
            ...
            const rowIndex = existingProductsMap.get(id);
            
            if (!rowIndex) {
                // Crear nuevo producto si no existe
                const newProductData = new Array(15).fill(''); // 15 columnas (A-O)
                
                // Mapear cada columna si existe en el archivo
                Object.keys(columnIndexes).forEach(key => {
                    const index = columnIndexes[key];
                    if (index !== -1 && row[index] !== undefined) {
                        const columnIndex = {
                            id: 0, producto: 1, gramos: 2, stock: 3, cantidadxgrupo: 4,
                            lista: 5, codigo_barras: 6, precios: 7, etiquetas: 8,
                            acopio_id: 9, alm_acopio_producto: 10, imagen: 11,
                            uSueltas: 12, promocion: 13, precio_promocion: 14
                        }[key];
                        
                        if (columnIndex !== undefined) {
                            newProductData[columnIndex] = row[index];
                        }
                    }
                });

                // Agregar el nuevo producto al final de la hoja
                await sheets.spreadsheets.values.append({
                    spreadsheetId,
                    range: 'Almacen general!A:O',
                    valueInputOption: 'USER_ENTERED',
                    resource: { values: [newProductData] }
                });
            ...
            const updateData = new Array(15).fill(''); // 15 columnas (A-O)
            ...
            Object.keys(columnIndexes).forEach(key => {
                ...
                if (index !== -1 && row[index] !== undefined) {
                    ...
                    if (columnIndex !== undefined) {
                        updateData[columnIndex] = row[index];
                    }
                }
            });
            ...
            await sheets.spreadsheets.values.update({
                spreadsheetId,
                range: `Almacen general!A${rowIndex}:O${rowIndex}`,
                valueInputOption: 'USER_ENTERED',
                resource: { values: [updateData] }
            });
...
app.post('/subir-acopio-excel', requireAuth, upload.single('archivo'), async (req, res) => {
    ...
    try {
        ...
        // Mapear índices de columnas para acopio
        const columnIndexes = {
            id: headers.findIndex(h => h === 'ID'),
            producto: headers.findIndex(h => h === 'PRODUCTO'),
            bruto: headers.findIndex(h => h === 'BRUTO (PESO-LOTE)'),
            prima: headers.findIndex(h => h === 'PRIMA (PESO-LOTE)'),
            etiquetas: headers.findIndex(h => h === 'ETIQUETAS')
        };

        // Verificar que las columnas requeridas estén presentes
        const columnasRequeridas = ['ID', 'PRODUCTO', 'BRUTO (PESO-LOTE)', 'PRIMA (PESO-LOTE)'];
        const columnasFaltantes = columnasRequeridas.filter(col => 
            headers.findIndex(h => h === col) === -1
        );
        ...
        // Obtener todos los productos de acopio existentes
        const existingProductsResponse = await sheets.spreadsheets.values.get({
            spreadsheetId,
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if (!rowIndex) {
                // Crear nuevo producto de acopio si no existe
                const newProductData = new Array(5).fill(''); // 5 columnas (A-E)
                ...
                Object.keys(columnIndexes).forEach(key => {
                    ...
                    if (index !== -1 && row[index] !== undefined) {
                        const columnIndex = {
                            id: 0, producto: 1, bruto: 2, prima: 3, etiquetas: 4
                        }[key];
            ...
            // Preparar los datos para actualizar (5 columnas A-E)
            const updateData = new Array(5).fill('');
            ...
            await sheets.spreadsheets.values.update({
                spreadsheetId,
                range: `Almacen acopio!A${rowIndex}:E${rowIndex}`,
                valueInputOption: 'USER_ENTERED',
                resource: { values: [updateData] }
            });
        ...
        console.error('Error al procesar archivo Excel de acopio:', error);

        res.status(500).json({
            success: false,
            error: 'Error al procesar el archivo Excel de acopio'
        });
    }
});
