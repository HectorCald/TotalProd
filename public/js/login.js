...
import { mostrarAnuncio, ocultarAnuncio, configuracionesEntrada, mostrarAnuncioSecond,mostrarAnuncioTercer, cerrarAnuncioManual, spinBoton, stopSpinBoton } from './modules/componentes/componentes.js'
...
function iniciarSesion() {
    ...
    if (loginButton) {
        loginButton.addEventListener('click', async () => {
            ...
            // Mostrar spinner y overlay
            spinBoton(loginButton);
...
function setupPaso3() {
    ...
    registerButton.addEventListener('click', async () => {
        ...
        try {
            ...
            const tipoApp = sessionStorage.getItem('registro_tipo_app');
            let response;

            if (tipoApp === 'ventas') {
                // Usar endpoint de usuarios ventas
                response = await fetch('/registrar-usuario-ventas', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                // Usar endpoint normal para empresas
                response = await fetch('/register', {
                    ...
                    body: JSON.stringify({
                        ...
                        password: password,
                        tipoApp: tipoApp,
                        empresaId: sessionStorage.getItem('registro_empresa') || null
                    })
                });
            }
